...
const Length UNBOUNDED_LENGTH = INFINITY;

bool isLengthUnbounded(Length length) {
    return std::isinf(length) == 1;
}

Size::Size(Length width, Length height) : width(width), height(height) {
    assert(!isLengthUnbounded(width) && !isLengthUnbounded(height));
...
Size Size::growBy(Length width, Length height) const {
...
Size Size::growToSatisfy(Constraints c) const {
    ...
    return {
        MAX(c.min.width, this->width),
        MAX(c.min.height, this->height),
    };
}
...
Size Size::shrinkBy(Length width, Length height) const {
    return {this->width - width, this->height - height};
}
...
Size Size::shrinkToSatisfy(Constraints c) const {
    ...
    return {
        MIN(c.max.width, this->width),
        MIN(c.max.height, this->height),
    };
}
...
Constraints Constraints::tightenTo(Length width, Length height) const {
...
Constraints Constraints::shrinkTo(Length width, Length height) const {
    return {
        this->max.width - width,
        this->max.height - height,
        MAX(0, this->min.width - width),
        MAX(0, this->min.height - height),
    };
}
...
Constraints Constraints::shrinkUnbounded(Length width, Length height) const {
    Length maxWidth = this->max.width;
    if (isLengthUnbounded(maxWidth)) {
        maxWidth = MAX(this->min.width, width);
    }
    Length maxHeight = this->max.height;
    if (isLengthUnbounded(maxHeight)) {
        maxHeight = MAX(this->min.height, height);
    }
...
Constraints Constraints::unboundedWidth() const {
    return {this->min, {UNBOUNDED_LENGTH, this->max.height}};
}

Constraints Constraints::unboundedHeight() const {
    return {this->min, {this->max.width, UNBOUNDED_LENGTH}};
}
...
Size Constraints::prefer(Length width, Length height) const {
    return {
        MAX(this->min.width, MIN(this->max.width, width)),
        MAX(this->min.height, MIN(this->max.height, height)),
    };
}
...
bool Constraints::contains(Length width, Length height) const {
...
Constraints Constraints::operator&&(const Constraints& that) const {
    return {
        MAX(this->min.width, that.min.width),
        MAX(this->min.height, that.min.height),
        MIN(this->max.width, that.max.width),
        MIN(this->max.height, that.max.height),
    };
}
